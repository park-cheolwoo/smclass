...
$(document).ready(function () {
    ...
    $('#layoutTable').on('click', '.modiBtn', function () {
        ...
        for (var i = 0; i < 7; i++) { // 0,1,6,7번째는 제외
            if (i == 0 || i == 1 || i == 6) {
                continue;
            }
    ...
    $('#layoutTable').on('click', '.svBtn', function () {
        ...
        // table의 column값에 영어/숫자 외에 다른 값이 들어있으면 alert
        // 2,5번째 row(이름, 주소)는 한글+영어+숫자만 입력가능
        // 3번째 row(이메일)은 @ 가 포함되어 있어야함
        // 4번째 row(전화번호)는 전화번호 형태'000-0000-0000'만 가능

        for (var i = 0; i < 4; i++) {
            if (i == 0 || i == 4) {
                if (!/^[가-힣a-zA-Z0-9]*$/.test(input_data[i])) {
                ...
                $.ajax({
                    headers: { "X-CSRFToken": csrfToken },
                    url: "/pros/updateMemberList/",
                    type: "POST",
                    data: {
                        'ID': row.find('td').eq(0).text(),
                        'NickName': input_data[0],
                        'Email': input_data[1],
                        'Phone': input_data[2],
                        'Address': input_data[3],
                    },
