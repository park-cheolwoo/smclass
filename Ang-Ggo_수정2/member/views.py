...
def changeInfoNewPwChk(request):
    if request.method == "GET":
        ...
        if newPw != newPwChk:
            return JsonResponse(
                {"result": "error", "message": "비밀번호 확인란을 정확히 입력하세요."}
            )
        ...
        try:
            ...
            if check_password(newPw, user.pw):
                return JsonResponse(
                    {
                        "result": "error",
                        "message": "기존 비밀번호와 동일합니다. 다른 비밀번호를 사용해 주세요.",
                    }
                )
            ...
            return JsonResponse(
                {"result": "error", "message": "사용자를 찾을 수 없습니다."}
            )
...
def joinAgree(request):
    if request.method == "GET":
        ...
        if agree1 == 1:
            ...
            Member.objects.create(agree1="필수약관 동의완료")

        if agree2 == 1:
            request.session["agree2"] = agree2

            Member.objects.create(agree2="선택약관 동의완료")

        today = datetime.now().strftime("%Y-%m-%d")
        context = {
            "result": "success",
            "today": today,
            "message": "회원약관에 동의하셨습니다",
        }
...
def emailDupChk(request):
    ...
    if Member.objects.filter(email=userEmail):
        return JsonResponse(
            {"result": "error", "message": "이미 사용 중인 이메일입니다."}
        )
...
def telDupChk(request):
    ...
    if Member.objects.filter(tel=userTel):
        return JsonResponse(
            {"result": "error", "message": "이미 사용 중인 전화번호입니다."}
        )
...
def nicknameDupChk(request):
    ...
    if Member.objects.filter(nickname=userNickname):
        return JsonResponse(
            {"result": "error", "message": "이미 사용 중인 닉네임입니다."}
        )
...
def idDupChk(request):
    ...
    if Member.objects.filter(id=userId):
        ...
        return JsonResponse(
            {"result": "error", "message": "이미 사용 중인 아이디입니다."}
        )
...
def newMember(request):
    if request.method == "GET":
        ...
        return JsonResponse(
            {"result": "success", "message": "회원가입이 완료되었습니다."}
        )
...
def verify_code(request):
    if request.method == "GET":
        ...
        if not input_code:
            return JsonResponse(
                {"result": "error", "message": "인증번호를 입력하세요."}
            )
        if not saved_code:
            return JsonResponse(
                {"result": "error", "message": "인증번호를 먼저 요청하세요."}
            )
        ...
        if input_code == saved_code:
            ...
            return JsonResponse(
                {"result": "error", "message": "인증번호가 일치하지 않습니다."}
            )
...
def send_verification_code(request):
    if request.method == "GET":
        ...
        try:
            ...
            return JsonResponse(
                {"result": "error", "message": "존재하지 않는 회원입니다."}
            )
        ...
        try:
            ...
            return JsonResponse(
                {"result": "success", "message": "인증번호가 이메일로 전송되었습니다."}
            )
        except Exception as e:
            return JsonResponse(
                {"result": "error", "message": f"메일 전송 실패: {str(e)}"}
            )
...
def findPassword(request):
    ...
    if request.method == "POST":
        ...
        if not name or not email:
            return JsonResponse(
                {"result": "error", "message": "이름과 이메일을 모두 입력하세요."}
            )
...
def findId(request):
    try:
        ...
        if qs.exists():
            ...
            return JsonResponse(
                {"result": "fail", "message": "존재하지 않는 회원입니다."}
            )
    except Exception as e:
        print(f"오류 발생 : {e}")
...
def loginChk(request):
    ...
    try:
        ...
        if check_password(pw, user.pw):
            ...
            list_qs = list(
                Member.objects.filter(id=id).values()
            )  # 사용자 정보 가져오기
            context = {
                "result": "success",
                "member": list_qs,
